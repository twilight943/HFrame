...
class DisRelGraphConv(nn.Module):
    ...
    def message(self, edges):
        ...
        dst = edges.data['dst']
        msg = th.cat((m, edges.data['etype'].view(-1, 1), dst.view(-1, 1)), dim=1)
        output, indices, count = th.unique(msg, dim=0, return_inverse=True, sorted=False, return_counts=True)
        indices = indices.new_empty(output.size(0)).scatter_(0, indices, th.arange(indices.size(0), dtype=indices.dtype, device=indices.device))
        zero_mask = th.zeros(m.size(0), device=m.device).view(-1, 1)
        zero_mask[indices] = 1
